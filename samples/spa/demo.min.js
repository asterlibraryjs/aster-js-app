...
});const IApplicationPart = ServiceIdentifier("IApplicationPart");var IAppConfigureHandler;
(function (IAppConfigureHandler) {
    function create(configure) {
        class CallbackAppConfigureHandler {
...
})(IAppConfigureHandler || (IAppConfigureHandler = {}));const INavigationHandler = ServiceIdentifier("INavigationHandler");const SEGMENT_SEPARATOR$1 = "/";
const WILDCARD_CHAR = "*";
const ASSIGN_CHAR = ":";
const NULLABLE_CHAR = "?";
...
}let ServiceRoutingHandler = class ServiceRoutingHandler {
    get route() { return this._route; }
...
ServiceRoutingHandler = __decorate([
    ServiceContract(IRoutingHandler)
], ServiceRoutingHandler);class ApplicationPartBuilder {
    constructor(partName, _source) {
        this._source = _source;
...
}const IRouter = ServiceIdentifier("IRouter");class RouteResolutionContext {
...
DefaultRouter = __decorate([
    ...
    __param(2, ILogger)
], DefaultRouter);function on(target, type, callback, options) {
...
], HyperlinkNavigationHandler);class DefaultApplicationConfigureHandler {
    configure(builder, host) {
        builder.configure(x => {
            x.addSingleton(HyperlinkNavigationHandler, { scope: 1 /* ServiceScope.container */ })
...
DefaultRendererService = __decorate([
    ...
    builder.addAction("/{bob}", IRendererService, (renderer, data) => renderer.render(`Bob is equal to: ${data.values["bob"]}`));
