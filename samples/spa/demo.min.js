...
})(ApplicationPartLifecycleHooks || (ApplicationPartLifecycleHooks = {}));/**
 * Service Id and implementation for the service in charge of handling the application routing
 */
const IRouter = ServiceIdentifier("IRouter");let DefaultNavigationHandler = class DefaultNavigationHandler {
    constructor(_router) {
        this._router = _router;
        this._self = this;
    }
    async [ApplicationPartLifecycleHooks.setup]() {
...
DefaultNavigationHandler = __decorate([
    ...
    [ApplicationPartLifecycleHooks.activated]() {
        if (!this._popstateHandle) {
            this._popstateHandle = on(window, "popstate", ev => this.onNavigate(ev));
        }
        return Promise.resolve();
    }
    onNavigate(ev) {
...
class Route {
    ...
    *[Symbol.iterator]() {
    ...
    static createDynamicSegment(name, optional, defaultValue = null) {
        if (name.startsWith("+")) {
...
Route.empty = new Route([]);const IRoutingHandler = ServiceIdentifier("IRoutingHandler");let ActionRoutingHandler = class ActionRoutingHandler {
...
ServiceRoutingHandler = __decorate([
    ServiceContract(IRoutingHandler)
], ServiceRoutingHandler);const IAsyncActionQueryMixin = q => class extends q {
    async each(callback) {
        var _a, e_1, _b, _c;
        let rank = 0;
        try {
...
class SortQuery extends QueryBase {
 ...
 */
const IContainerRouteData = ServiceIdentifier("IContainerRouteData");
let ContainerRouteData = class ContainerRouteData {
    constructor() {
...
(function (ParamValues) {
    ...
    ParamValues.create = create;
})(ParamValues || (ParamValues = {}));const SEGMENT_SEPARATOR = "/";
let DefaultRouter = class DefaultRouter {
    constructor(_handlers, _application, _routeData, _logger) {
    ...
    *getHandlers() {
    ...
    getActiveChildren(nested) {
        return __asyncGenerator(this, arguments, function* getActiveChildren_1() {
            const active = this._application.activeChild;
            if (active) {
                let router = active.services.get(IRouter);
                if (router) {
                    yield yield __await(router);
                    if (nested)
                        yield __await(yield* __asyncDelegator(__asyncValues(router.getActiveChildren(true))));
                }
            }
        });
    }
    ...
    async invokeHandler(handler, ctx, values, query) {
        ...
        try {
            for (var _d = true, _e = __asyncValues(this.getActiveChildren(false)), _f; _f = await _e.next(), _a = _f.done, !_a; _d = true) {
                _c = _f.value;
                _d = false;
                const router = _c;
                if (await router.handle(ctx, mergedValues, query))
                    break;
            }
        }
        catch (e_1_1) { e_1 = { error: e_1_1 }; }
...
Tag.lazy("route args", () => new Lookup());class SetupIoCContainerBuilder {
    ...
    addController(ctor) {
        ...
        this.configure(x => {
            ...
            x.addScoped(serviceId, ctor, { scope: 1 /* ServiceScope.container */ });
            for (const handler of handlers) {
                x.addScoped(IRoutingHandler, handler, { scope: 1 /* ServiceScope.container */ });
            }
        });
        return this;
    }
    ...
    constructor(descriptor, serviceProvider) {
        this._instance = serviceProvider.get(descriptor, true);
    }
    [ApplicationPartLifecycleHooks.setup]() {
...
ApplicationPartLifecycleWrapper = __decorate([
    ServiceContract(IApplicationPartLifecycle),
    __param(1, IServiceProvider)
], ApplicationPartLifecycleWrapper);class ApplicationPart extends DisposableHost {
    ...
    configureMandatoryAppPartServices(services) {
        services.addInstance(IApplicationPart, this, { scope: 1 /* ServiceScope.container */ });
        for (const desc of this.extractImplicitLifecycleImpl(services)) {
            services.addTransient(ApplicationPartLifecycleWrapper, { baseArgs: [desc], scope: 1 /* ServiceScope.container */ });
        }
    }
    extractImplicitLifecycleImpl(services) {
    ...
    async activatePart(part) {
        if (this._current)
            await this.invokeLifecycleHook(this._current, ApplicationPartLifecycleHooks.deactivated);
        await this.invokeLifecycleHook(part, ApplicationPartLifecycleHooks.activated);
    }
    async invokeLifecycleHook(part, hook) {
        const promises = [];
