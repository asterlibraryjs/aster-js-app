...
__decorate([
    Memoize
], AsyncEventEmitter.prototype, "event", null);class InstanciationContext {
    constructor(target) {
        this.target = target;
...
})(ServiceEntry || (ServiceEntry = {}));let InstantiationService = class InstantiationService {
        ...
        for (const param of ServiceIdentifier.registry.dependencies(ctor)) {
            if (param.type === "many") {
                ...
                if (found) {
                    ...
                    throw new Error(`No service named "${param.serviceId}" registered ${ctor.name}`);
                }
                else {
...
class AllSettledPromiseError extends AssertionError {
    ...
    get running() { var _a; return !((_a = this._token) === null || _a === void 0 ? void 0 : _a.aborted); }
    ...
    async start() {
        ...
        try {
            ...
            for (const setup of setupCallbacks) {
                ...
                if (result === IoCModuleSetupResultBehavior.stop)
                    break;
            }
...
}class IoCModuleBuilder extends IoCContainerBuilder {
...
let ConsoleLogger = class ConsoleLogger {
    constructor(_name, _level, _clock) {
        this._name = _name;
        this._level = _level;
        this._clock = _clock;
    }
    log(logLevel, message, err) {
...
let ConsoleLoggerFactory = class ConsoleLoggerFactory {
    constructor(_level, _module, _clock) {
        this._level = _level;
...
ConsoleLoggerFactory.targetType = ConsoleLogger;
ConsoleLoggerFactory = __decorate([
    ServiceFactory(ILogger),
    __param(1, IIoCModule),
    __param(2, IClock)
], ConsoleLoggerFactory);function addConsoleLogger(level) {
...
Object.assign(ServiceCollection.prototype, {
...
});const IApplicationPart = ServiceIdentifier("IApplicationPart");const IRouter = ServiceIdentifier("IRouter");const IRoutingHandler = ServiceIdentifier("IRoutingHandler");class RouteResolutionContext {
    ...
    *[Symbol.iterator]() {
    ...
    async evalCore(actions) {
        for (const [handler, values] of actions) {
            try {
                await handler.handle(values, this._application);
            }
            catch (err) {
                this._logger.log(4 /* LogLevel.error */, "Error handled during route handler invocation", err);
            }
        }
    }
    *resolveActions(ctx) {
...
let ScopedRouter = class ScopedRouter extends DefaultRouter {
    constructor(_parent, handlers, application, logger) {
...
ScopedRouter = __decorate([
    ServiceContract(IRouter),
    __param(1, Many(IRoutingHandler)),
    __param(2, IApplicationPart),
    __param(3, ILogger)
], ScopedRouter);const INavigationHandler = ServiceIdentifier("INavigationHandler");function on(target, type, callback, options) {
    target.addEventListener(type, callback, options);
    return IDisposable.create(() => {
        target.removeEventListener(type, callback, options);
    });
}const IViewRender = ServiceIdentifier("IViewRender");let HyperlinkNavigationHandler = class HyperlinkNavigationHandler {
    constructor(_router, _renderer) {
        this._router = _router;
        this._renderer = _renderer;
    }
    start() {
        this._registration = on(this._renderer.root, "click", ev => this.onRootClick(ev));
    }
    onRootClick(ev) {
        alert(0);
...
HyperlinkNavigationHandler = __decorate([
    ...
    __param(0, IRouter),
    __param(1, IViewRender)
], HyperlinkNavigationHandler);class Route {
    ...
    isMatch(ctx) {
        if (ctx.remaining > this._segments.length && !this._wildcard)
            return false;
        if (this._isRelative) {
            if (ctx.remaining === ctx.initialSize)
                return false;
        }
    ...
    static *parseRoute(route) {
        ...
        for (let segment of segments) {
            if (segment === "*")
                return WildcardRouteSegment.instance;
            if (segment.startsWith(":")) {
                const params = segment.substring(1).split("?");
                if (params.length == 2) {
...
class StaticRouteSegment {
    constructor(_segment, _optional) {
    ...
    loadValues(ctx, values) { }
}
class WildcardRouteSegment {
    constructor() { }
    isMatch(segment) { return true; }
    loadValues(ctx, values) { }
}
...
}let ActionRoutingHandler = class ActionRoutingHandler {
...
], ActionRoutingHandler);class SetupIoCContainerBuilder {
...
}let ApplicationPartBuilder = class ApplicationPartBuilder {
    constructor(partName, source, _router) {
        this._router = _router;
...
ApplicationPartBuilder = __decorate([
    __param(2, Optional(IRouter))
], ApplicationPartBuilder);let ApplicationPart = class ApplicationPart {
    get name() { return this._name; }
    get parent() { return this._parent; }
    get running() { return this._module.running; }
    get ready() { return this._module.ready; }
    get abortToken() { return this._module.abortToken; }
    get services() { return this._module.services; }
    constructor(_name, _parent, builder) {
        this._name = _name;
        this._parent = _parent;
        this._module = builder
            .configure(x => x.addInstance(this, { scope: 1 /* ServiceScope.container */ }))
            .build();
...
ApplicationPart = __decorate([
    ServiceContract(IApplicationPart)
], ApplicationPart);var SinglePageApplication_1;
let SinglePageApplication = SinglePageApplication_1 = class SinglePageApplication extends ApplicationPart {
    constructor(appName, handlerCtor) {
        const kernel = SinglePageApplication_1.createKernel();
        const builder = SinglePageApplication_1.createApp(kernel, appName, handlerCtor);
    ...
    static createApp(kernel, appName, handlerCtor) {
    ...
    static async start(appName, handlerCtor) {
        const app = new SinglePageApplication_1(appName, handlerCtor);
        await app.start();
        return app;
    }
};
SinglePageApplication = SinglePageApplication_1 = __decorate([
    ServiceContract(IApplicationPart)
], SinglePageApplication);class DataService {
}class MainConfigureHandler {
    configure(builder) {
        builder.configure(x => x.addSingleton(DataService));
        builder.addAction("/{bob}", x => alert(x));
    }
}SinglePageApplication.start("Library", MainConfigureHandler);//# sourceMappingURL=demo.min.js.map
