...
let DefaultLogger = class DefaultLogger {
    ...
    trace(message) {
        this.log(0 /* LogLevel.trace */, message);
    }
    debug(message) {
        this.log(1 /* LogLevel.debug */, message);
    }
    info(message) {
        this.log(2 /* LogLevel.info */, message);
    }
    warn(message, err) {
        this.log(3 /* LogLevel.warn */, message, err);
    }
    error(message, err) {
        this.log(4 /* LogLevel.error */, message, err);
    }
    critical(message, err) {
        this.log(5 /* LogLevel.critical */, message, err);
    }
};
...
let ConsoleLoggerSink = class ConsoleLoggerSink {
    ...
    log({ scope, time, logLevel, message, err }) {
...
ServiceRoutingHandler = __decorate([
    ServiceContract(IRoutingHandler)
], ServiceRoutingHandler);class SetupIoCContainerBuilder {
...
__decorate([
    Memoize
], ApplicationPart.prototype, "ready", null);class ApplicationPartBuilder {
...
HyperlinkNavigationHandler = __decorate([
    ServiceContract(INavigationHandler),
    __param(0, IRouter)
], HyperlinkNavigationHandler);class RouteResolutionContext {
...
DefaultRouter = __decorate([
    ...
    __param(2, ILogger)
], DefaultRouter);class DefaultApplicationConfigureHandler {
    configure(builder, host) {
        builder.configure(x => {
...
DefaultRendererService = __decorate([
    ...
    builder.addAction("/:bob/*", IRendererService, (renderer, data) => renderer.render(`Dédé a un petit: ${data.values["bob"]}`));
